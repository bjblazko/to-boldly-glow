...
describe('scaledDistanceUnits', () => {
  ...
  it('matches the log1p compact scale at blend 1', () => {
    expect(scaledDistanceUnits(5, 1)).toBeCloseTo(compactDistanceUnits(5), 10)
  })

  it('interpolates linearly between the two endpoints at blend 0.5', () => {
    const realistic = 5 * AU_TO_SCENE_UNITS
    const compact = compactDistanceUnits(5)
    expect(scaledDistanceUnits(5, 0.5)).toBeCloseTo((realistic + compact) / 2, 10)
  })

  it('compresses far distances more than near ones in compact mode', () => {
    ...
    const ratioCompact = scaledDistanceUnits(neptuneAu, 1) / scaledDistanceUnits(mercuryAu, 1)
    expect(ratioCompact).toBeLessThan(ratioRealistic)
  })
})
...
describe('geometricBlend', () => {
  ...
  it('matches the compact endpoint at blend 1', () => {
...
describe('scaledBodyRadiusUnits', () => {
  ...
  it('interpolates geometrically (not linearly) between the two endpoints at blend 0.5', () => {
    ...
    const compact = 1.0
    const atHalf = scaledBodyRadiusUnits(6371, compact, 0.5, AU_KM)
    expect(atHalf).toBeCloseTo(Math.sqrt(realistic * compact), 10)
  })
})
